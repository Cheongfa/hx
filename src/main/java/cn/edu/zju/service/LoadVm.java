...
public class LoadVm {
    ...
    public static List<VmEntity> parseXml() throws Exception {

        List<VmEntity> vmEntityList = new LinkedList<VmEntity>();
        // step 1:获得DOM解析器工厂
        // 工厂的作用是创建具体的解析器
        ...
        document = db.parse(new File("E:\\ideaworkspaces\\format\\src\\main\\java\\vmconf.xml"));
        ...
        /*    String content0 = element.getElementsByTagName("ip").item(0)
            ...
            String content = element.getElementsByTagName("job").item(0)
                    .getFirstChild().getNodeValue();
            System.out.println("job: " + content);// 此处打印出job
            vmEntity.setJob(content);
            // 后面类似处理即可：
            content = element.getElementsByTagName("index").item(0)
                    .getFirstChild().getNodeValue();
            System.out.println("index: " + content);
            int index = Integer.parseInt(content);
            vmEntity.setIndex(index);
            content = element.getElementsByTagName("ip").item(0)
                    .getFirstChild().getNodeValue();
            System.out.println("ip: " + content);

            vmEntity.setIp(content);
            vmEntityList.add(vmEntity);
